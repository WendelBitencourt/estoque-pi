...
function buildLista(produtos: Produto[], lotes: Lote[]): { urgentes: ItemNecessidade[]; atencao: ItemNecessidade[] } {
  ...
  for (const p of produtos) {
    ...
    const item: ItemNecessidade = { produtoId: p.id, nome: p.nome, emoji: p.emoji, total, risco, diasPiorLote: dias, motivo };
...
function gerarTextoWhatsApp(urgentes: ItemNecessidade[], atencao: ItemNecessidade[]): string {
  ...
  const linhas = [`🏠 *Casa da Criança — Lista de Necessidades*`, `📅 ${hoje}`, ''];
  if (urgentes.length > 0) {
    linhas.push('🔴 *URGENTE — precisa de doação agora:*');
    for (const i of urgentes) linhas.push(`  ${i.emoji} ${i.nome} (${i.motivo})`);
    linhas.push('');
  }
  if (atencao.length > 0) {
    linhas.push('🟡 *Atenção — vai precisar em breve:*');
    for (const i of atencao) linhas.push(`  ${i.emoji} ${i.nome} (${i.motivo})`);
    linhas.push('');
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.itemInfo}>
        <Text style={[styles.itemNome, { color: colors.textPrimary }]} numberOfLines={1}>{item.nome}</Text>
        ...
        <Text style={[styles.itemMotivo, { color: item.risco === 'risco_alto' ? colors.riscoAltoDark : colors.riscoAtencaoDark }]}>
          {item.motivo}
        </Text>
      </View>
...
export default function NecessidadesScreen() {
  ...
  function handleCompartilhar() {
    ...
    Share.share({ message: texto }).catch(() => {
      Alert.alert('Compartilhar', 'A lista foi preparada. O compartilhamento via WhatsApp estará disponível na próxima fase.', [{ text: 'OK' }]);
    });
  }
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[styles.subtitulo, { color: colors.textSecondary }]}>
              {total === 0 ? 'Tudo em ordem no estoque!' : `${total} ${total === 1 ? 'produto precisa' : 'produtos precisam'} de doação`}
            </Text>
          </View>
        </View>
              ...
              <View style={styles.secao}>
                <View style={styles.secaoHeaderRow}>
                  ...
                  <View style={[styles.secaoCount, { backgroundColor: colors.riscoAltoLight }]}>
                    <Text style={[styles.secaoCountTexto, { color: colors.riscoAltoDark }]}>{urgentes.length}</Text>
                  </View>
                </View>
                <Text style={[styles.secaoDesc, { color: colors.textSecondary }]}>Precisam de doação o quanto antes</Text>
              ...
              <View style={styles.secao}>
                <View style={styles.secaoHeaderRow}>
                  ...
                  <View style={[styles.secaoCount, { backgroundColor: colors.riscoAtencaoLight }]}>
                    <Text style={[styles.secaoCountTexto, { color: colors.riscoAtencaoDark }]}>{atencao.length}</Text>
                  </View>
                </View>
                <Text style={[styles.secaoDesc, { color: colors.textSecondary }]}>Vencimento próximo — providencie em breve</Text>
                <View style={styles.lista}>
...
const styles = StyleSheet.create({
  ...
  whatsappBtn: {
    ...
    padding: 18, borderRadius: 18, marginBottom: 28,
    ...Platform.select({
  ...
  dicaCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, padding: 16, borderRadius: 16, borderWidth: 1, marginTop: 4 },
