...
export default function HistoricoScreen() {
  ...
  const secoes = useMemo(() => {
    ...
    return Object.entries(grupos)
      ...
      .map(([chave, data]) => ({ title: chave, data: data.sort((a, b) => b.data.localeCompare(a.data)) }));
      ...
      <View style={[styles.header, { backgroundColor: colors.background }]}>
        <View style={styles.headerTop}>
          ...
          <TouchableOpacity
            ...
            onPress={() => Alert.alert('Exportar PDF', 'A exportação de relatório estará disponível na próxima fase.')}
            activeOpacity={0.8}
          >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
          {FILTROS.map((f) => {
            ...
            const bgAtiva = f.valor === 'entrada' ? colors.riscoSeguro : f.valor === 'saida' ? colors.primary : f.valor === 'descarte' ? colors.riscoAlto : colors.primary;
            return (
              <TouchableOpacity
                key={f.valor}
        ...
        <SectionList
          ...
          renderSectionHeader={({ section }) => (
            <View style={[styles.secaoHeader, { backgroundColor: colors.background }]}>
              <Text style={[styles.secaoTitulo, { color: colors.textSecondary }]}>{labelMes(section.title).toUpperCase()}</Text>
              <View style={[styles.secaoLinha, { backgroundColor: colors.divider }]} />
              <Text style={[styles.secaoCount, { color: colors.textDisabled }]}>{section.data.length} mov.</Text>
            </View>
          )}
...
function ResumoCard({ emoji, label, valor, bg, cor }: { emoji: string; label: string; valor: number; bg: string; cor: string }) {
  return (
